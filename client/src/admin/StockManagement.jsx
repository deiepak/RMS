...
export default function StockManagement() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [stockRes, vendorsRes, requestsRes, menuRes] = await Promise.all([
  ...
  const handleDismissRequest = async (id) => {
    try {
      ...
      showToast('Request marked as Okay', 'success');
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="admin-content">
      <div className="admin-header">
        ...
        <div className="flex gap-sm">
          ...
          <button className="btn btn-primary" onClick={() => openModal()}>
            <Plus size={18} /> New {activeTab === 'kitchen' ? 'Kitchen' : 'General'} Item
          </button>
        </div>
      </div>
      ...
      {stockRequests.length > 0 && (
        <div className="card mb-lg" style={{ border: '1px solid var(--warning)' }}>
          <div className="card-header bg-secondary">
            <h3 className="flex align-center gap-sm text-warning"><AlertTriangle size={18} /> Kitchen Stock Requests</h3>
          </div>
            ...
            <tbody>
              {stockRequests.map(req => (
                <tr key={req.id}>
                  ...
                  <td>{req.quantity}</td>
                  <td className="text-secondary">{req.notes || '-'}</td>
                  <td className="text-right">
                    <button className="btn btn-sm btn-primary" onClick={() => handleDismissRequest(req.id)}>Okay (Clear)</button>
                  </td>
                </tr>
              ))}
            </tbody>
